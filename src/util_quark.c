...
word_count *create_word_count( int id ) {
  ...
  result->links = g_hash_table_new_full(&g_int_hash, &g_int_equal, free, free);
  return result;
}

/******************************************************************************/

int *get_id( GHashTable *symbols_dict, GHashTable *inv_symbols_dict, char *entry_buff, int entry_len ){
...
void insert_into_index( GHashTable *hash, char *w1, char *w2, double count, 
                        int *id_counter ) {
  GQuark *idw1 = malloc( sizeof(GQuark) );
  *idw1 = g_quark_from_string( w1 );
  GQuark *idw2 = malloc( sizeof(GQuark) );
  *idw2 = g_quark_from_string( w2 );
  //perra("%s:%d - %s:%d\n",w1,*idw1,w2,*idw2);
