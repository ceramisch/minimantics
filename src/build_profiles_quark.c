  ...
  while( !feof( input ) ) {
    ...
    if( r == 3 && !feof( input ) ) {
        if( t_len <= MAX_S && c_len <= MAX_S ) {
            //perra( "Target:\"%s\" Context:\"%s\"\n",target_buff,context_buff );        
            insert_into_index( t_dict, target_buff, context_buff, count, &idc_t );
            insert_into_index( c_dict, context_buff, target_buff, count, &idc_c );
            n_pairs += count;
        }
        else {
...
void calculate_and_print_am( int *w1, int idw1, int *w2, int idw2, 
  ...
  const char *w1s = g_quark_to_string( *w1 );
  const char *w2s = g_quark_to_string( *w2 );  
